...
if [ ! -e "/etc/first.runed" ]; then
    ...
    /home/update_file.php "$domain" "$MYSQL_ROOT_PASSWORD" "$MYSQL_MAIL_PASSWORD" "$URL" "$WEBMAIL_URL" "$MAIL_DB_HOST" "$MAIL_DB_ROOT_USERNAME"
...
#      初始化ewomail数据
if [ ! -d "/ewomail/mail/mysql" ]; then
    mkdir -p /ewomail/mail/mysql
    echo '初始化ewomail数据'
    ...
    /home/init_sql.php "$domain" "$MYSQL_ROOT_PASSWORD" "$MYSQL_MAIL_PASSWORD" "$MAIL_DB_HOST" "$MAIL_DB_ROOT_USERNAME"
fi
