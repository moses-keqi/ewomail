...
class init_sql{
    ...
    public function __construct($domain,$root_pwd,$mail_pwd,$mail_db_host, $mail_db_root_username){
        ...
        $this->domain = $domain;
        $this->db = new mysqli($mail_db_host, $mail_db_root_username,$root_pwd,'mysql');
...
//         $this->db->query("UPDATE user SET password=PASSWORD('$root_pwd') WHERE user='root'");
    ...
    public function update_mail_config()
    {
        ...
        $this->db->query("create user rainloop@% identified by 'rainloop'");
        $this->db->query("grant all privileges on rainloop.* to rainloop@%");
        $this->db->query("flush privileges");
    }
...
#TODO
$init_sql = new init_sql($argv[1],$argv[2],$argv[3],$argv[4],$argv[5]);
$init_sql->db->close();
?>